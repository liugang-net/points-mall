...
class PointsMall::AdminScoreEventController < Admin::AdminController
  ...
  def index
    ...
    events = events.joins(:user).where("users.username = ?", params[:username]) if params[:username].present?
  ...
  def create
    ...
    raise Discourse::InvalidParameters.new(I18n.t("points_mall.admin.score_events.user_not_found")) unless user
    ...
    event =
      DiscourseGamification::GamificationScoreEvent.new(
        ...
        description: params[:description] || I18n.t("points_mall.admin.score_events.manual_adjustment"),
      )
